        ...
        if verbose and step % (epoch_size // 1) == 0:
            if lm_phase_id == 0:
                ...
                print("[" + time.strftime('%Y-%m-%d %H:%M:%S') + "] "
                      ...
                      % (step * 1.0 / epoch_size, np.exp(costs_phrase_p / iters_phrase_p),
                         num_word / (time.time() - start_time)))
            else:
                print("[" + time.strftime('%Y-%m-%d %H:%M:%S') + "] "
                      "%.3f phrase ppl: %.3f speed: %.0f wps"
                      % (step * 1.0 / epoch_size, np.exp(costs_phrase / iters_phrase),
                         num_word / (time.time() - start_time)))
            sys.stdout.flush()
    costs_list = np.exp(costs / iters), np.exp(costs_phrase_p / iters_phrase_p), np.exp(costs_phrase / iters_phrase)
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=gpu_config) as session:
            ...
            # It contains three phases: lm_train_phase, phrase_prob_train_phase and phrase_train_phase
            # If you don't want to train phrase, just skip the phrase_prob_train_phase and phrase_train_phase.
            if FLAGS.use_phrase:
                lm_phase_num = 3
            else:
                lm_phase_num = 1
