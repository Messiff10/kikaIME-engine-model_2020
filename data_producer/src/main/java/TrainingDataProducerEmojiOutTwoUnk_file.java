 ...
 * 直接读文件，不是文件夹
 */
public class TrainingDataProducerEmojiOutTwoUnk_file
{
//
	//en
...
//	private static final String PUN_REREX = "[^aábcčdďeéěfghchiíjklmnňoópqrřsštťuúůvwxyýzžAÁBCČDĎEÉĚFGHChIÍJKLMNŇOÓPQRŘSŠTŤUÚŮVWXYÝZŽ0-9']";
	...
	//ko
//	private static final String WORD_REGEX = "[ㅂㄷㅈㄱㅃㄸㅉㄲㅍㅌㅊㅋㅅㅎㅆㅁㄴㅇㄹㅣㅔㅚㅐㅏㅗㅜㅓㅡㅢㅖㅒㅑㅛㅠㅕㅟㅞㅙㅘㅝ']+";
//	private static final String NUM_REGEX = "[+-]*[0-9]+.*[0-9]*";
//	private static final String PUN_REREX = "[^ㅂㄷㅈㄱㅃㄸㅉㄲㅍㅌㅊㅋㅅㅎㅆㅁㄴㅇㄹㅣㅔㅚㅐㅏㅗㅜㅓㅡㅢㅖㅒㅑㅛㅠㅕㅟㅞㅙㅘㅝ0-9']";
	//fr
...
//		int wordsNum = 20000;
		int phraseNum =20000;
		String dataPathIn = "/Users/ff/Desktop/train_data/"+s+"/de_user_web_case_pro.txt";
...
//            for (String word : wordsCountMapFromData.sort().keySet()) {//降序所有单词
//                if (wordsCountMapFromVocab.keySet().contains(word)) {
//                    if ( smallWordsDictMap.size() < wordsNum) {
//                        smallWordsDictMap.put(word, wordsCountMapFromData.get(word));//2w词表
//                        if (!emojisSet.contains(word)) {
//                            for (int i = 0; i < word.length(); i++) {
//                                dictLettersSet.add(String.valueOf(word.charAt(i)));//letter词表
//                            }
//                        }
//                    }
//                }
...
//			System.out.println("phraseCountMapFromData size"+phraseCountMapFromData.size());
//
//            for (String phrase : phraseCountMapFromData.sort().keySet()){
...
//        map.put("letter", letter);
	...
	private static void convertData(String dataPathIn, double rateThreshold, int lineNum,int trainDataNum,
		...
		try {
			...
			BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(dataPathIn),
					"UTF-8"));
...
//        System.out.println("wordidmap size:"+phraseIdMap.size());
...
//            writerPhrase.close();
	...
	private static void convertToIds(String dataPathIn, String dataPathOut) {
		convertToIdsFile(wordIdMap, outWordIdMap, letterIdMap, dataPathIn + "/train_data",
				dataPathOut + "/train_in_ids_lm", dataPathOut + "/train_in_ids_letters");
//                dataPathOut + "/train_ids_phrase");
		convertToIdsFile(wordIdMap, outWordIdMap, letterIdMap, dataPathIn + "/dev_data",
